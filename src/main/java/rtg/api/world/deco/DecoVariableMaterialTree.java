...
/*
 * Author @Zeno410
 * This class chooses a birch, oak, or spruce tree using the materials picker and then calls the appropriate variable tre
 */
...
public class DecoVariableMaterialTree extends DecoTree {
    ...
    protected DecoVariableOak oakTree = new DecoVariableOak();
    protected DecoVariableSpruce spruceTree = new DecoVariableSpruce();
    protected DecoVariableTree birchTree = new DecoVariableBirch();

    public DecoVariableMaterialTree(TreeMaterials.Chooser chooser) {
        this.chooser = chooser;
    }

    public void changeAverageHeightSqrt(float change) {
        oakTree.changeAverageHeightSqrt(change);
        spruceTree.changeAverageHeightSqrt(change);
        birchTree.changeAverageHeightSqrt(change);
    }

    public void changeHeightNoiseVariability(float change) {
        oakTree.changeHeightNoiseVariability(change);
        spruceTree.changeHeightNoiseVariability(change);
        birchTree.changeHeightNoiseVariability(change);
    }
    ...
    public void generate(final IRealisticBiome biome, final RTGWorld rtgWorld, final Random rand, final ChunkPos chunkPos, final float river, final boolean hasVillage, ChunkInfo chunkInfo) {
        ...
        if (event.getResult() != Event.Result.DENY) {
            ...
            while (treesRemaining.notDone()) {
                ...
                // with current designs oak trees are wider so if oak try again
                if (materials.equals(TreeMaterials.Picker.oak)) {
                    materials = chooser.materialFor(pos, rtgWorld, rand);
                }

                if (materials.equals(TreeMaterials.Picker.oak)) {
                    oakTree.doVariableGenerate(rand, chunkInfo, pos, y, treesRemaining);
                } else if (materials.equals(TreeMaterials.Picker.birch)) {
                    birchTree.doVariableGenerate(rand, chunkInfo, pos, y, treesRemaining);
