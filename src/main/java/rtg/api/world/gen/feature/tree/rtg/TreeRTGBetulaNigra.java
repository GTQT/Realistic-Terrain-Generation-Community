...
public class TreeRTGBetulaNigra extends TreeRTG {
    ...
    private float interbranchLengthing = .01f; // 2/3s of the lengthening of interbranch for each additional branch;
    ...
    public TreeRTGBetulaNigra(TreeRTGBetulaNigra model) {
    ...
    public boolean generate(World world, Random rand, BlockPos pos) {
        ...
        while (height >0) { // top down
        	...
        	float ascent = 1.0f + height/(float)crownSize*.2f + rand.nextFloat()*0.2f;
	...
	public float estimatedSize() {
    	...
    	float branchLength= this.shortestBranch + this.branchLengthening*((float)crownSize)/3f;
	...
	private void makeLeafyLogBranch (RTGTreeBranch branch, World world, Random rand, SkylightTracker lightTracker) {
		...
		if (branch.stage == 2) {
			...
			// these are in brackets to prevent misnaming bugs
			{
				...
				float  leftLength = 2.0f *(.9f + rand.nextFloat()*0.2f);
				RTGTreeBranch leftBranch = new RTGTreeBranch(leftDirection,leftAscent,leftLength,branch.stage+1,branch.location());
			...
			}
			{
				...
				float  rightLength =  2.0f *(.9f + rand.nextFloat()*0.2f);
				RTGTreeBranch rightBranch = new RTGTreeBranch(rightDirection,rightAscent,rightLength,branch.stage+1,branch.location());
